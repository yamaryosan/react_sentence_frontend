        ...
        <>
            ...
            {deviceType === 'mobile' && (
                <>
                    ...
                    <Box sx={{display: 'flex', flexDirection: 'column', backgroundColor: 'secondary.light'}}>
                        {pages.map((page) => (
                            <React.Fragment key={page.name}>
                                <Button href={page.url} sx={{color: 'black', width: '100%', gap: '0.5rem', fontSize: '1.2rem', borderBottom: '1px solid black'}}>
                                    {page.icon}
                                    {page.name}
                                </Button>
                            </React.Fragment>
